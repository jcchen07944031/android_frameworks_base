...
public class QuickStatusBarHeader extends RelativeLayout implements
    ...
    protected void onFinishInflate() {
        ...
        Dependency.get(TunerService.class).addTunable(this,
                mQsPanel.QS_SHOW_AUTO_BRIGHTNESS, mQsPanel.QS_SHOW_BRIGHTNESS_SLIDER,
                mQsPanel.QS_SHOW_BRIGHTNESS_BUTTONS);
    ...
    public void onTuningChanged(String key, String newValue) {
        if (mQsPanel.QS_SHOW_BRIGHTNESS_SLIDER.equals(key)) {
            try {
                mIsQuickQsBrightnessEnabled = Integer.parseInt(newValue) > 0;
            } catch (NumberFormatException e) {
                // Catches exception as newValue may be null or malformed.
                mIsQuickQsBrightnessEnabled = false;
            }
            updateResources();
        } else if (mQsPanel.QS_SHOW_AUTO_BRIGHTNESS.equals(key)) {
            mIsQsAutoBrightnessEnabled = TunerService.parseIntegerSwitch(newValue, true);
            updateResources();
        } else if (mQsPanel.QS_SHOW_BRIGHTNESS_BUTTONS.equals(key)) {
            mBrightnessButton = TunerService.parseIntegerSwitch(newValue, false);
            updateResources();
        }
    }
